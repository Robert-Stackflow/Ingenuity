...
public class MsgListAdapter<MESSAGE extends IMessage> extends RecyclerView.Adapter<ViewHolder>
    ...
    // File message
    private final int TYPE_SEND_FILE = 10;
    private final int TYPE_RECEIVE_FILE = 11;

    //NameCard message
    private final int TYPE_SEND_NAMECARD = 12;
    private final int TYPE_RECEIVER_NAMECARD = 13;

    // Group change message
    private final int TYPE_EVENT = 14;
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        switch (viewType) {
            ...
            case TYPE_SEND_NAMECARD:
                return getHolder(parent, mHolders.mSendNameCardLayout, mHolders.mSendNameCardHolder, true);
            case TYPE_RECEIVER_NAMECARD:
                return getHolder(parent, mHolders.mReceiveNameCardLayout, mHolders.mReceiveNameCardHolder, false);
            case TYPE_EVENT:
    ...
    public int getItemViewType(int position) {
        ...
        if (wrapper.item instanceof IMessage) {
            ...
            if (message.getType() == IMessage.MessageType.EVENT) {
                return TYPE_EVENT;
            } else if (message.getType() == IMessage.MessageType.SEND_TEXT) {
                return TYPE_SEND_TXT;
            } else if (message.getType() == IMessage.MessageType.RECEIVE_TEXT) {
                return TYPE_RECEIVE_TXT;
            } else if (message.getType() == IMessage.MessageType.SEND_IMAGE) {
                return TYPE_SEND_IMAGE;
            } else if (message.getType() == IMessage.MessageType.RECEIVE_IMAGE) {
                return TYPE_RECEIVER_IMAGE;
            } else if (message.getType() == IMessage.MessageType.SEND_VOICE) {
                return TYPE_SEND_VOICE;
            } else if (message.getType() == IMessage.MessageType.RECEIVE_VOICE) {
                return TYPE_RECEIVER_VOICE;
            } else if (message.getType() == IMessage.MessageType.SEND_VIDEO) {
                return TYPE_SEND_VIDEO;
            } else if (message.getType() == IMessage.MessageType.RECEIVE_VIDEO) {
                return TYPE_RECEIVE_VIDEO;
            } else if (message.getType() == IMessage.MessageType.SEND_FILE) {
                return TYPE_SEND_FILE;
            } else if (message.getType() == IMessage.MessageType.RECEIVE_FILE) {
                return TYPE_RECEIVE_FILE;
            } else if (message.getType() == IMessage.MessageType.SEND_LOCATION) {
                return TYPE_SEND_LOCATION;
            } else if (message.getType() == IMessage.MessageType.RECEIVE_LOCATION) {
                return TYPE_RECEIVER_LOCATION;
            } else if (message.getType() == IMessage.MessageType.SEND_NAMECARD) {
                return TYPE_SEND_NAMECARD;
            } else if (message.getType() == IMessage.MessageType.RECEIVE_NAMECARD) {
                return TYPE_RECEIVER_NAMECARD;
            }
        }
        return TYPE_SEND_TXT;
    }
...
//            mMsgClickListener.onMessageClick(view, message);
    ...
    public interface OnMsgClickListener<MESSAGE extends IMessage> {
        void onMessageClick(View view, MESSAGE message, BaseMessageViewHolder<MESSAGE> viewHolder);
    }
    ...
    public static class HoldersConfig {
        ...
        private Class<? extends BaseMessageViewHolder<? extends IMessage>> mSendNameCardHolder;
        private Class<? extends BaseMessageViewHolder<? extends IMessage>> mReceiveNameCardHolder;
        ...
        private int mSendNameCardLayout;
        private int mReceiveNameCardLayout;
        ...
        public HoldersConfig() {
            ...
            mSendNameCardHolder = DefaultNameCardViewHolder.class;
            mReceiveNameCardHolder = DefaultNameCardViewHolder.class;
            ...
            mReceiveTxtLayout = R.layout.item_receive_text;
            ...
            mSendNameCardLayout = R.layout.item_send_namecard;
            mReceiveNameCardLayout = R.layout.item_receive_namecard;
    ...
    private static class DefaultNameCardViewHolder extends NameCardViewHolder<IMessage> {

        public DefaultNameCardViewHolder(View itemView, boolean isSender) {
            super(itemView, isSender);
        }
    }
