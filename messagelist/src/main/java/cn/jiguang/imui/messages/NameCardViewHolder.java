...
public class NameCardViewHolder<Message extends IMessage> extends BaseMessageViewHolder<Message> implements MsgListAdapter.DefaultMessageViewHolder {
    ...
    private final CircleImageView mImageCover;
    ...
    public NameCardViewHolder(View itemView, boolean isSender) {
        super(itemView);
        ...
        mImageCover = itemView.findViewById(R.id.aurora_tv_namecard_avatar);
        mTvDescribe = itemView.findViewById(R.id.aurora_tv_namecard_describe);
        mTvName = itemView.findViewById(R.id.aurora_tv_namecard_name);
        mContainer = itemView.findViewById(R.id.aurora_tv_namecard_container);
        if (isSender) {
    ...
    public void onBind(@NonNull final Message message) {
        ...
        if (message.getNameCardUser() != null) {
            mTvName.setText(message.getNameCardUser().getDisplayName());
            mTvDescribe.setText(message.getNameCardUser().getDescribe());
            if (mImageLoader != null) {
                if (message.getNameCardUser().getAvatarFilePath() != null && !message.getNameCardUser().getAvatarFilePath().isEmpty()) {
                    mImageLoader.loadImage(mImageCover, message.getNameCardUser().getAvatarFilePath(), 5, true, true, true, true);
                }
            }
        }
