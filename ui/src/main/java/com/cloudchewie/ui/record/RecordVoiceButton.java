...
public class RecordVoiceButton extends AppCompatImageButton {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        float[] values = new float[]{1.0f, 1.1f, 1.2f, 1.3f, 1.4f, 1.5f, 1.45f, 1.4f, 1.35f, 1.3f, 1.25f, 1.2f, 1.15f, 1.1f, 1.05f, 1.0f};
        float[] restoreValues = new float[]{1.0f};
        set = new AnimatorSet();
        ObjectAnimator xAnimator = ObjectAnimator.ofFloat(this, "scaleX", values);
        xAnimator.setRepeatCount(1000);
        ObjectAnimator yAnimator = ObjectAnimator.ofFloat(this, "scaleY", values);
        yAnimator.setRepeatCount(1000);
    ...
    public void setVoiceFilePath(String path, String fileName) {
        if (null == path || TextUtils.isEmpty(path) || null == fileName || TextUtils.isEmpty(fileName)) {
    ...
    public void finishRecord(boolean isPreview) {
        ...
        if (intervalTime < MIN_INTERVAL_TIME) {
            IToast.makeTextBottom(getContext(), mContext.getString(R.string.time_too_short_toast), Toast.LENGTH_SHORT).show();
...
//                    });
                ...
                if (mp != null) {
                    ...
                    if (mDuration < 1)
                        mDuration = 1;
                    if (null != mListener)
                        mListener.onFinishRecord(myRecAudioFile, mDuration, isPreview);
//                    if (isPreview) {
...
//                    }
                    ...
                    IToast.makeTextBottom(mContext, mContext.getString(R.string.record_voice_permission_request), Toast.LENGTH_SHORT).show();
                }
            }
        }
    }
    ...
    private void startRecording() {
        try {
            ...
            e.printStackTrace();
            IToast.makeTextBottom(mContext, mContext.getString(R.string.illegal_state_toast), Toast.LENGTH_SHORT).show();
            cancelTimer();
            dismissDialog();
            if (mThread != null) {
                mThread.exit();
            ...
            e.printStackTrace();
            IToast.makeTextBottom(mContext, mContext.getString(R.string.aurora_record_voice_file_not_exist), Toast.LENGTH_SHORT).show();
        } catch (RuntimeException e) {
            IToast.makeTextBottom(mContext, mContext.getString(R.string.record_voice_permission_denied), Toast.LENGTH_SHORT).show();
            cancelTimer();
            dismissDialog();
            if (mThread != null) {
                mThread.exit();
