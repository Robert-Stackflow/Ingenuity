...
class ReadableBottomBar @JvmOverloads constructor(
    ...
    init {
        val typedArray = context.theme.obtainStyledAttributes(
            attrs, R.styleable.ReadableBottomBar, defStyleAttr, defStyleAttr
        )
        ...
        val activeItemType = ItemType.getType(
            typedArray.getInt(
                R.styleable.ReadableBottomBar_rbb_activeItemType, ItemType.Icon.value
            )
        )
    ...
    fun setTabInitialSelectedIndex(tabInitialSelectedIndex: Int) {
        this.tabInitialSelectedIndex = tabInitialSelectedIndex
    }
    ...
    private fun drawBottomBarItems() {
        ...
        bottomBarItemList.forEach { item ->
            val bottomBarItem = BottomBarItemView(context).apply {
                layoutParams =
                    LayoutParams(itemWidth.toInt(), itemHeight.toInt() - tabIndicatorHeight)
    ...
    private fun animateIndicator(currentItemIndex: Int) {
        val previousMargin: Float =
            (indicatorView?.layoutParams as? LayoutParams)?.leftMargin?.toFloat() ?: 0F
