...
public class CropImageView extends AppCompatImageView {
    ...
    private void initImage() {
        ...
        float midPointX = viewWidth / 2.0F;
        float midPointY = viewHeight / 2.0F;
        ...
        mFocusRect.left = mFocusMidPoint.x - mFocusWidth / 2.0F;
        mFocusRect.right = mFocusMidPoint.x + mFocusWidth / 2.0F;
        mFocusRect.top = mFocusMidPoint.y - mFocusHeight / 2.0F;
        mFocusRect.bottom = mFocusMidPoint.y + mFocusHeight / 2.0F;
        ...
        //图像中点为中心进行缩放
        matrix.setScale(scale, scale, mImageWidth / 2.0F, mImageHeight / 2.0F);
        float[] mImageMatrixValues = new float[9];
        matrix.getValues(mImageMatrixValues); //获取缩放后的mImageMatrix的值
        float transX = mFocusMidPoint.x - (mImageMatrixValues[2] + mImageWidth * mImageMatrixValues[0] / 2.0F);  //X轴方向的位移
        float transY = mFocusMidPoint.y - (mImageMatrixValues[5] + mImageHeight * mImageMatrixValues[4] / 2.0F); //Y轴方向的位移
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (Style.RECTANGLE == mStyle) {
            ...
            float radius = Math.min((mFocusRect.right - mFocusRect.left) / 2.0F, (mFocusRect.bottom - mFocusRect.top) / 2.0F);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction() & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_POINTER_DOWN:  //第二个点按下
                ...
                midPoint.set((pA.x + pB.x) / 2.0F, (pA.y + pB.y) / 2.0F);
            ...
            case MotionEvent.ACTION_POINTER_UP:
                if (mode == DRAG) {
                    ...
                    int rotateLevel = (int) Math.floor((rotation + Math.PI / 4) / (Math.PI / 2.0F));
    ...
    public Bitmap rotate(Bitmap bitmap, int degrees) {
        if (degrees != 0 && bitmap != null) {
            ...
            matrix.setRotate(degrees, (float) bitmap.getWidth() / 2.0F, (float) bitmap.getHeight() / 2.0F);
            try {
        ...
        try {
            ...
            if (expectWidth != width || exceptHeight != height) {
                ...
                if (mStyle == CropImageView.Style.CIRCLE && !isSaveRectangle) {
                    ...
                    canvas.drawCircle(expectWidth / 2.0F, exceptHeight / 2.0F, radius, paint);
                    bitmap = circleBitmap;
                }
            }
