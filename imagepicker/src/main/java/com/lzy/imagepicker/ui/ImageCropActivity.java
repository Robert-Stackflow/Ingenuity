...
public class ImageCropActivity extends ImageBaseActivity implements View.OnClickListener, CropImageView.OnBitmapSaveCompleteListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (imagePicker.getFocusWidth() == 0) {
            int with = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 280, getResources().getDisplayMetrics());
            mCropImageView.setFocusWidth(with);
        } else {
            mCropImageView.setFocusWidth(imagePicker.getFocusWidth());
        }

        if (imagePicker.getFocusHeight() == 0) {
            int height = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 280, getResources().getDisplayMetrics());
            mCropImageView.setFocusWidth(height);
        } else {
