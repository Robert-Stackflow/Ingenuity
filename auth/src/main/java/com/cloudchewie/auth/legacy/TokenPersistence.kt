...
class TokenPersistence @Inject constructor(@ApplicationContext private val ctx: Context) {
    ...
    private suspend fun getToken(id: String): Token? = withContext(Dispatchers.IO) {
    ...
    private suspend fun save(token: Token) = withContext(Dispatchers.IO) {
    ...
    companion object {
        val TAG: String = TokenPersistence::class.java.simpleName
