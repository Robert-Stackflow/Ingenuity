...
import com.cloudchewie.auth.legacy.SavedTokens
import com.cloudchewie.auth.legacy.Token
import com.cloudchewie.auth.legacy.TokenPersistence
...
class MigrationUtil @Inject constructor(
    ...
    suspend fun convertLegacySavedTokensToOtpTokens(savedTokens: SavedTokens): List<OtpToken> =
        withContext(Dispatchers.IO) {
            val tokenMap = savedTokens.tokens.associateBy { token ->
                token.id
            }
    ...
    private suspend fun convertLegacyTokensToOtpTokens(legacyTokens: List<Token>): List<OtpToken> =
        withContext(Dispatchers.IO) {
    ...
    suspend fun convertOtpTokensToLegacyTokens(tokens: List<OtpToken>) =
        withContext(Dispatchers.IO) {
            tokens.map {
